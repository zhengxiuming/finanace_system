...
const columns = tableData.tableColumns.accountDetail;
const data = [{
  key: '1',
...
//修饰器
@TimeFilter
class RecordedDetail extends React.Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    const {list} = this.props.basicLayout;
        ...
        <Card hoverable={true}>
          <Form onSubmit={this.handleSubmit}>
            ...
            <div className={styles.search_item}>
              <span>支付方式：</span>
              <FormItem className={styles.formItem}>
                {getFieldDecorator('pay_type', {initialValue: [`${query.pay_type}`]})(
                  <Select
                    placeholder="请选择支付方式"
              ...
              <FormItem className={`${styles.formItem} ${styles.formOuter}`}>
                {getFieldDecorator('area_code')(
                  ...
                  >
                    {list && list.map(item => <Option key={item.area_code}
                                                      value={item.area_code}>{item.area_name}</Option>)}
                  </Select>
                )}
              </FormItem>
              <FormItem className={`${styles.formItem} ${styles.formOuter}`}>
                {getFieldDecorator('key')(
                  <Input style={{width: '300px'}} placeholder="请输入医院编号/医院名称/经销商名称"/>
                )}
              </FormItem>
...
RecordedDetail = Form.create()(RecordedDetail);
export default connect(({recordedDetail, basicLayout, loading}) => ({
  recordedDetail,
  basicLayout,
  loading
}))(RecordedDetail);
